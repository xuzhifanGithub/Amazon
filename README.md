...
KataGo 引擎、所需 DLL，以及 L、X 两套模型；目标电脑不需要另行安装
...
Linux / macOS 可运行纯 Python GUI 和人人对弈；L/X 需要在目标平台
自行编译对应原生模块和引擎：
...
1. **两个 AI 对弈选项**：菜单「游戏 → 黑方/白方 → AI」可为双方独立选择 L 或 X。
2. **显示胜率**（仿照参考 Hex 项目）：
...
5. **自由摆盘**：使用「游戏 → 自由摆盘…」(Ctrl+B)，选择黑方皇后、白方皇后、
   ...
   自定义初始局面会写入 JSON 棋谱，重新导入后仍能继续对局或演示；L、X 均受支持。
