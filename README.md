...
一款带有 L/X 两套神经网络模型、三阶段胜率分析与桌面宠物的 PyQt6 亚马逊棋客户端。
...
- **四个 AI 入口**：`MCTS★`、`MCTS-18特征★★`、`amazon_L★★★` 与
  `amazon_X★★★★`，黑白双方均可独立选择。
...
C++ MCTS 模块、KataGo 引擎、所需 DLL，以及 L、X 两套模型；目标电脑不需要另行安装
...
仍可进行人人对弈或使用两个 C++ MCTS。

Linux / macOS 可运行纯 Python GUI 和人人对弈；C++ MCTS 与 L/X 需要在目标平台
自行编译对应原生模块和引擎：
...
需要本机安装 Python 3.10、3.11 或 3.13。
缺少某个原生模块、模型或配置时，对应 AI 菜单会自动禁用，程序仍可进行人人对弈。

## AI 引擎说明

- **MCTS★**：基础 C++ 蒙特卡洛树搜索，使用分阶段公式估值器。
- **MCTS-18特征★★**：搜索框架与基础 MCTS 相同，叶节点使用 18 特征局面估值器。
- **X（自训练模型，默认）**：
...
- **amazon_L（后备模型）**：
  - 使用同一个可读取外部权重的 `src/ai/kataAmazonEngineCuda/amazons.exe`
    ...
    `amazons8x-b20c256-s182755840-d39606896`；
    以后直接替换此路径下的兼容模型会真实生效。旧模型偶尔会把亚马逊棋着法返回为
    `pass`，桥接层会改选搜索结果中访问量最高的合法坐标。服务器曾使用的 `initial_fixed`
    与该模型张量相同，因此不再作为 Z 重复展示。
- **默认自动选择**：`amazons_engine.py` 检测到 X 的完整资源则优先使用，否则回退到 L；
  GUI 菜单可为黑白双方独立选择。
- **模型**：`src/ai/kataAmazonEngineCuda/amazon10x10_xzf.bin.gz`
  ...
  X 的默认对局搜索配置使用 600 visits。菜单「AI 参数设置」提供默认均衡和
  最强棋力预设；最强棋力使用 600 visits KataGo，并为 X/L
...
> 也不会计算新增输入特征，输出不可信；原始 L 是 amazons8x 模型，
> 桥接层会修正其在低 visits 时偶发的非法 `pass` 返回。替换 L 权重时需自行确保配套引擎兼容其格式。
...
1. **四个 AI 对弈选项**：菜单「游戏 → 黑方/白方 → AI」可为双方独立选择两个
   C++ MCTS、amazon_L 或 amazon_X。
2. **显示胜率**（仿照参考 Hex 项目）：
...
5. **自由摆盘**：使用「游戏 → 自由摆盘…」(Ctrl+B)，选择黑方皇后、白方皇后、
   ...
   自定义初始局面会写入 JSON 棋谱，重新导入后仍能继续对局或演示；四个 AI 均受支持。
