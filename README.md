...
本仓库当前**未授予开源许可证**，默认版权保护适用。项目内的模型文件仅随本应用提供
本地推理与对弈使用；未授权将模型用于训练、微调、蒸馏、格式转换、权重提取或单独再分发。
第三方引擎、运行库、字体和素材仍分别受其原始许可证及权利声明约束。

公开仓库意味着 GitHub 用户仍可查看和 fork 仓库。若需要商业使用、再发布或其他授权，
请通过仓库所有者的 GitHub 主页联系。
